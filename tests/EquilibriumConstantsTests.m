...
classdef EquilibriumConstantsTests < matlab.unittest.TestCase
     methods (Static)
         function calculated = calculate_all_static(inputs)
             ...
             composition = Composition(salinity).set_silicate_concentration(silicate).set_phosphate_concentration(phosphate).set_ammonia_concentration(ammonia).set_sulphide_concentration(sulphide).estimate_all_from_salinity(which_boron).remove_freshwater_species(which_ks).adjust_geosecs_species(which_ks).calculate_peng_correction(which_ks);
