classdef KFunctions
    methods (Static=true)
        ...
        function kc = calculate_surface_kc(temp_c,salinity,which_ks,pH_scale_conversion)
            temp_k = temp_c + 273.15;
            log_temp_k = log(temp_k);

            sqrt_salinity = sqrt(salinity);

            KC = nan(numel(temp_c),1);
            
            selected = (which_ks.k1_k2~=6 & which_ks.k1_k2~=7);

            % CalciteSolubility:
            % '       Mucci, Alphonso, Amer. J. of Science 283:781-799, 1983.
            logKCa = (-171.9065 - 0.077993.*temp_k(selected) + 2839.319./temp_k(selected)...
                  + 71.595.*log_temp_k(selected)./log(10)...
                  + (-0.77712 + 0.0028426.*temp_k(selected) + 178.34./temp_k(selected)).*sqrt_salinity(selected)...
                  - 0.07711.*salinity(selected) + 0.0041249.*sqrt_salinity(selected).*salinity(selected));
            KC(selected) = 10.^(logKCa);% ' this is in (mol/kg-SW)^2
            ...
            KC(selected) = 0.0000001.*(-34.452 - 39.866.*salinity(selected).^(1./3) +...
                110.21.*log(salinity(selected))./log(10) - 0.0000075752.*temp_k(selected).^2);
        ...
        function kc_pressure_correction = calculate_pressure_correction_kc(which_ks,temp_c,pressure_bar)
            temp_k    = temp_c + 273.15;
            gas_constant = Constants.gas_constant;

            kc_pressure_correction = NaN(numel(pressure_bar),1);
            ...
            selected = (which_ks.k1_k2~=6 & which_ks.k1_k2~=7);            
            deltaV = -48.76 + 0.5304.*temp_c(selected);
            Kappa  = (-11.76 + 0.3692.*temp_c(selected))./1000;
            lnKCafac  = (-deltaV + 0.5.*Kappa.*pressure_bar(selected)).*pressure_bar(selected)./(gas_constant.*temp_k(selected));
            kc_pressure_correction(selected) = exp(lnKCafac);
        ...
        function kc = calculate_kc(temp_c,pressure_bar,salinity,which_ks,pH_scale_conversion)
            kc_surface = KFunctions.calculate_surface_kc(temp_c,salinity,which_ks,pH_scale_conversion(1));
